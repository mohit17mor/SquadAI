...
test("launches Windows command shims through cmd.exe without using a shell", () => {
  assert.deepEqual(createCodexLaunchSpec(
...
class FakeTransport implements AppServerTransport {
  ...
  server(message: JsonRpcMessage): void {
  ...
  async waitForRequest(method: string): Promise<JsonRpcMessage> {
  ...
  clientResponseFor(serverRequestId: number): JsonRpcMessage {
  ...
  async waitForClientResponse(serverRequestId: number): Promise<JsonRpcMessage> {
    const deadline = Date.now() + 1_000;
    while (Date.now() < deadline) {
      const response = this.sent.find((message) => message.id === serverRequestId && !message.method);
      if (response) {
...
async function startedSession(transport: FakeTransport) {
...
test("initialize advertises experimental API capability for dynamic tools", async () => {
  ...
  try {
    assert.deepEqual((initialize.params as Record<string, unknown>).capabilities, {
...
test("approval handler can pause and approve command execution requests", async () => {
  const transport = new FakeTransport();
  const captured: { request?: ApprovalRequest } = {};
  const resolver: {
    approve?: (value: { decision: "approved"; reason: string }) => void;
  } = {};
  ...
  assert.equal(
    transport.sent.some((message) => message.id === 994 && !message.method),
    false,
  );
