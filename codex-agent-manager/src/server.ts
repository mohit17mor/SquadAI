...
async function pickWindowsDirectory(defaultPath: string): Promise<string | null> {
  const script = [
    ...
    "      dialog.SetTitle(\"Choose agent workspace\");",
    ...
    "      if ((uint)result == 0x800704C7) return null;",
    "      Marshal.ThrowExceptionForHR(result);",
    "      IShellItem selected; dialog.GetResult(out selected);",
    "      IntPtr displayName; selected.GetDisplayName(0x80058000, out displayName);",
    "      try { return Marshal.PtrToStringUni(displayName); } finally { Marshal.FreeCoTaskMem(displayName); }",
    "    } finally { Marshal.FinalReleaseComObject(dialog); }",
    "  }",
    "}",
    "'@",
    "Add-Type -TypeDefinition $source",
    "$selectedPath = [ModernFolderPicker]::Pick($initialPath)",
